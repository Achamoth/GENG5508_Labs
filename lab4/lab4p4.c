  ...
  for(int i=0; i<numClear; i++) {
    for(int j=i+1; j<numClear; j++) {
      ...
      if(!intersection(i, j)) {
        ...
        paths[numPaths] = (int *) malloc(sizeof(int) * 6);
...
void qadd(int *queue, int *qcurr, int *qend, int elem) {
  queue[*qend] = elem;
  *qend = *qend + 1;
}

int qget(int *queue, int *qcurr, int *qend) {
  int elem = queue[*qcurr];
  *qcurr = *qcurr + 1;
  return elem;
}

int qempty(int *queue, int *qcurr, int *qend) {
  return *qcurr == *qend;
}
...
int main(int argc, char * argv[]) {
  ...
  //Convert clear array to an adjacency matrix
  int adjMatrix[numClear][numClear];
  for(int i=0; i<numClear; i++) {
    for(int j=0; j<numClear; j++) {
      adjMatrix[i][j] = -1;
    }
  }
  for(int i=0; i<numPaths; i++) {
    adjMatrix[paths[i][4]][paths[i][5]] = i;
  }

  int istart;
  int iend;
  //Find top left-most and bottom right-most clear quads
  for(int i=0; i< numClear; i++) {
    if(clear[i][0] == 0 && clear[i][1] == 0) {
      //Top left
      istart = i;
    }
    if(clear[i][0] + clear[i][2] == 128 && clear[i][1] + clear[i][2] == 128) {
      //Bottom right
      iend = i;
    }
  }

  //Now, perform dijkstra's algorithm to find path between start and end clear quads
  int queue[10000];
  int qcur = 0;
  int qend = 0;
  int visited[numClear];
  for(int i=0; i<numClear; i++) visited[i] = -1; //Mark all as unvisited to start off with

  //Add start node
  qadd(queue, &qcur, &qend, istart);
  visited[istart] = 0;

  printf("Starting path finding\n");

  while(!qempty(queue, &qcur, &qend)) {
  	//Get next elem
    int current = qget(queue, &qcur, &qend);
    if(current == iend) break;
    //add not visisted neighbours
    for(int i = 0; i < numClear; i++) {
      if(visited[i] >= 0) continue;
      //check path
      if(adjMatrix[current][i] == -1) continue;
      //add to queue
      qadd(queue, &qcur, &qend, i);
      //mark as visited + 1
      visited[i] = visited[current] + 1;
    }
  }

  printf("End is %d jumps\n", visited[iend]);
  ...
  int rcurrent = iend;
  //Path reconstruction
  while(rcurrent != istart) {
    int dist = visited[rcurrent];
    for(int i = 0; i < numClear; i++) {
      if(visited[i] < 0) continue;
      if(visited[i] == dist - 1) {
        int pathid = adjMatrix[rcurrent][i];
        qadd(rpath, &rstart, &rend, pathid);
        LCDLine(paths[pathid][0], paths[pathid][1], paths[pathid][2], paths[pathid][3], 0x0000ff);
        rcurrent = i;
        break;
      }
    }
